...
TFMPI2C tfmP(&tfBus);         // Create a TFMini-Plus I2C object

void setup(){
    SerialUSB.begin( 115200);   // Initialize terminal serial port
    delay(20);

    tfBus.begin();            // Initialize two-wire interface
//    tfBus.setClock( 400000);  // Set I2C bus speed to 'Fast' if
...
void loop(){
    ...
    if( tfmP.status == TFMP_READY){
      SerialUSB.print(tfmP.status);
      SerialUSB.print(tfDist);   // display distance,
      SerialUSB.println(tfTemp);   // display signal strength/quality,[0]
        // printf( "Temp:%2i%s",  tfTemp, "°C" );   // display temperature,
        // printf( "\r\n");                   // end-of-line.
    }
    SerialUSB.println(tfmP.status);
    delay(100);    //  Run loop at approximately 20Hz.
}
// = = = = = = = = =  End of Main Loop  = = = = = = = = =
