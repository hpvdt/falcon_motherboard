...
bool TFMPI2C::getData( int16_t &dist, int16_t &flux, int16_t &temp, uint8_t addr)
{
    ...
    {
      if( tfBus->peek() == -1)     // If there is no next byte...
      {
        status = TFMP_I2CREAD;   // then set error...
        return false;            // and return "false."
      }
      else frame[ i] = uint8_t( tfBus->read());
    }
...
bool TFMPI2C::sendCommand( uint32_t cmnd, uint32_t param, uint8_t addr)
{
    ...
    tfBus->beginTransmission( addr);
    // Queue command data array for transmission to the I2C device
    if( tfBus->write( cmndData, (size_t)cmndLen) != cmndLen)
    {
        status = TFMP_I2CLENGTH;  // then set satus code...
        tfBus->write( 0);           // Put a zero in the xmit buffer.
    ...
    tfBus->requestFrom( (int)addr, (int)replyLen, true);

    memset( reply, 0, sizeof( reply));   // Clear the reply data buffer.
    for( uint8_t i = 0; i < replyLen; i++)
    {
      reply[ i] = (uint8_t)tfBus->read();
    }
...
// waiting for a transfer to finish.  This function bypasses the tfBus
...
// that had been in progress.  It concludes by calling `tfBus->begin()`.
//
